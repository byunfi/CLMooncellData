...
enum MooncellURL: String, URLConvertible {
    static let domian = "https://fgo.wiki/index.php?title="
    case home = "首页"
    case eventList = "活动一览"
    
    func asURL() throws -> URL { URL(string: "\(MooncellURL.domian)\(rawValue)&mobileaction=toggle_view_desktop".addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!)!
    }
}

open class MooncellExtractor {
...
/// Extractor for the home page of `Mooncell`.
open class HomeME: MooncellExtractor {
    
    private enum ContentSection: String, CustomStringConvertible {
    ...
    private func extract(in contentSection: ContentSection, target sourceType: SourceType) -> [MCHomeBox] {
    ...
    open func extractNewData(target sourceType: SourceType) -> [MCHomeBox] {
        return extract(in: .newCards, target: sourceType)
    }
    
    open func extractEvents(target sourceType: SourceType) -> [MCHomeBox] {
        return extract(in: .events, target: sourceType)
    }
    
    open func extractSummons(target sourceType: SourceType) -> [MCHomeBox] {
        return extract(in: .summons, target: sourceType)
    }
    
    open func extractMasterMission(target sourceType: SourceType) -> MCMasterMission {
...
class EventListME: MooncellExtractor {
    ...
    func extarctFutureEvents() -> [MCEventListItem] {
        ...
        return html.xpath(trXPath).dropFirst().map { tr -> MCEventListItem in
            ...
            return MCEventListItem(openDate: String(openDate), closeDate: String(closeDate), name: name,
                                   href: href, imageSrc: imageSrc, type: type, officialHref: officialHref)
        }
    }
}
